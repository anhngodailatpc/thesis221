...
function Register(props: any) {
  ...
  useEffect(() => {
    ...
    async function getActivity() {
      try {
        const { data, count } = await activityRegistrationApi.get(filters)
        setCount(count)
        setData(data)
      } catch (error) {
        console.error(error)
  ...
  return (
    <>
      <CCard>
        <CCardHeader>Đăng ký hoạt động</CCardHeader>
        <CCardBody>
          <CRow className='mb-2'>
          ...
          <CDataTable
            ...
            scopedSlots={{
              ...
              action: (item: any) => {
                let elementAction: any = 'Xảy ra lỗi'
                if (item.numberRegisteredSuccess >= item.maximumParticipant) {
                  elementAction =
                    item.statusRegistration === 'PASS'
                      ? 'Thành công'
                      : 'Không được nhận'
                } else {
                  switch (item.statusRegistration) {
                    case 'PASS':
                      elementAction = 'Thành công'
                      break
                    case 'CANCEL':
                    case '':
                      elementAction = (
                        <CButton
                          ...
                          disabled={
                            !(
                              moment().diff(
                                moment(item?.registerStartDay),
                                'seconds'
                              ) >= 0 &&
                              moment().diff(
                                moment(item?.registerEndDay),
                                'seconds'
                              ) <= 0
                            )
                          }
                ...
                return (
                  <td className='align-middle text-center'>{elementAction}</td>
                )
              },
            }}></CDataTable>
