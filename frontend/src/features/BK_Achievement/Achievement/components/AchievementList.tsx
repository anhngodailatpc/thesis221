    ...
    <>
      <CDataTable
        ...
        scopedSlots={{
          ...
          action: (item: any) => {
            const emailManageUnit = item.manageUnit.map(
              (ob: string) => ob.split(',')[0]
            )
            return (
              <>
                <td className='d-flex justify-content-start'>
                  <>
                    {emailManageUnit.includes(user.email) && (
                      <>
                        <CTooltip content='Danh sách quản lý thành viên đơn vị'>
                          <CButton
                            size='sm'
                            color='light'
                            className='ml-1 mt-1'
                            onClick={() => {
                              const str = item.manageUnit.find(
                                (ob: string) => ob.split(',')[0] === user.email
                              )
                              history.push(
                                `/thanh-vien-don-vi/${str.split(',')[1]}&${
                                  item.id
                                }`
                              )
                            }}>
                    ...
                    {roles.MANAGER.includes(user.role) &&
                      item.status === 'Trạng thái chờ' && (
                        <>
                          <CTooltip content='Cập nhật thông tin Hồ sơ'>
                            <CButton
                              size='sm'
                              color='info'
                              className='ml-1 mt-1'
                          ...
                          <CTooltip content='Quản lý đơn vị'>
                            <CButton
                              size='sm'
                              color='light'
                              className='ml-1 mt-1'
                              onClick={() => {
                                history.push(`/quan-ly-don-vi/${item.id}`)
                              }}>
                              <CIcon size='sm' content={freeSet.cilAlignLeft} />
                            </CButton>
                          </CTooltip>
                          <CTooltip content='Xóa Hồ sơ'>
                            <CButton
                              size='sm'
                              color='danger'
                              className='ml-1 mt-1'
                    ...
                    {item.status === 'Đã kết thúc' && (
                      <>
                        ...
                        {roles.MANAGER.includes(user.role) &&
                          item.lock === 'forever' && (
                            <CTooltip content='Tổng kết Hồ sơ'>
                              <CButton
                                size='sm'
                                color='warning'
                                className='ml-1 mt-1'
                        ...
                        {(roles.MANAGER === user.role ||
                          ...
                          <>
                            <CTooltip content='Danh sách cán bộ nộp hồ sơ'>
                              <CButton
                                size='sm'
                                color='info'
                                className='ml-1 mt-1'
