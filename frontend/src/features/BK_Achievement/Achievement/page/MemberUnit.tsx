...
const MemberUnit = ({ match }: RouteComponentProps<TParam>) => {
  const achievementId = parseInt(match.params.id.split('&')[1])
  const codeDepartment = match.params.id.split('&')[0]
  ...
  useEffect(() => {
    ...
    async function getUsers() {
      try {
        const { data: users, count } = await UserApi.getFilterMemberUnit(
          filters,
          achievementId+'',
          codeDepartment
        )
  ...
  return (
    <>
      {[roles.MANAGER, roles.PARTICIPANT].includes(currentUser.role) && (
        <CCard>
          <CCardBody>
            <CRow className='mb-2'>
                ...
                status: (item: any) => {
                  return (
                    <td className='align-middle text-center'>
                      {item.isSubmission ? 'Đã nộp' : 'Chưa nộp'}
                    </td>
                  )
                },
                action: (item: any) => {
