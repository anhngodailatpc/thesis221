...
const ShowHistoryCompetition = () => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const { data, count } = await achievementApi.getWithFilter(filters,false,'COMPETITION');

        const customCompetitionList = data.map((item: Achievement) => {
          return {
            ...item,
            status: "Đã kết thúc",
          };
        ...
        setCompetitionList(customCompetitionList);
        setCount(count);
      } catch (error: any) {
        console.error(error);
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        ...
        const newList = competitionList.filter((item) => {
...
export default ShowHistoryCompetition;
